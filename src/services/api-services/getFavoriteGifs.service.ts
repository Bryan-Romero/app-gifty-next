...
import { getGifsByID } from '../giphy-services/getGifsByID.service'

export async function getFavoriteGifs({ token, ...params }: PaginationProps & { token: string }): Promise<Gifs | null> {
  try {
    const res = await ApiClient.get<ApiRes<PaginationRes<string>>>('favorites/pagination-gifId', {
      params,
